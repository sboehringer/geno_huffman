...
public:
	...
	Genotypes(char *filename, int Nid1, int Nsnps1) : Nid(Nid1), Nsnps(Nsnps1) {//read input file
		char *file = filename;
		FILE *in = fopen("hapmap-ceu.bed", "r");
		if (!in){
...
//===========================================================
  ...
  bool correlation (const int snp1, const int snp2,  std::vector<int> &row1, std::vector<int> &row2, const int Number_of_ind, const int Use_correlation) {// calculate correlation betweeen two snps
  //if R^2 > 0,81 return true
	  double Threshold = 0;
	  if (Use_correlation == 0){
	   Threshold = 5;
	  }
	  else{
	   Threshold = 0.81;
 ...
 calculate_prob(prob_matrix, symbol_probability, max_a, max_b, key, row, matrix_freq, a, b, s);
  ...
  void blocking_across_snps (const int window_size, const int Number_of_ind, const int Number_of_snps, Huff_tree <int, double> &tree, Huff_tree <int, double> &min_tree, 
	  double maf_max, double maf_min, ofstream &result, ofstream &ind_result, const int Use_correlation ){ //
	  ...
	  while ((position_snp < Number_of_snps ) || ((int) window.size() > 0)){ 
		...
		int i=current_snp+1;
		 //fill set_cor_snp by correlated snps
		  while (((int)set_cor_snp.size() < 4) && i < (int)window.size()){

	 	    if( correlation (current_snp, i, window[current_snp], window[i], Number_of_ind, Use_correlation)){
...
//#if 0
	 ...
	 if (argc < 13) { // Check the value of argc. If not enough parameters have been passed, inform user and exit.
        std::cout << "Usage is -in <input file name> -maf_min <> -maf_max <> -snp <number of snps> -ind <number of individuals> -cor <0 - do not use correlation between snps>\n"; 
        std::cin.get();
        exit(0);
...
//const int Number_of_snps = 2239393;
 ...
 int Use_correlation = (int)atof(cor);
//#endif
    //char file1 = *"hapmap-ceu.bed";
	...
	//int Number_of_snps = 2239392;
	//int Number_of_ind = 60;
	...
	ofstream result("Huffman_result.txt");  
	//make a head of output file
	result << "Correlated snps\tfrequencies\tblocks\tlength\n";
	ofstream ind_result("Huffman_ind_result.txt");
	...
	genotype->blocking_across_snps (window_size, Number_of_ind, Number_of_snps, *tree, *min_tree, atof(maf_max), atof(maf_min), result, ind_result, Use_correlation);
